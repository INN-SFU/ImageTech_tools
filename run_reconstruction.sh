...
# Automatically enable BIDS_MRI if refacing is requested
if [ $REFACE -eq 1 ]; then
    BIDS_MRI=1
fi
...
#while IFS= read -r subject_id_in_subject_file; do
#    echo "$subject_id_in_subject_file"
...
#        subj_base=$(echo "$subject_id_in_subject_file" | sed -E 's/^sub[_-]//' | sed -E 's/^([A-Za-z0-9]+).*/\1/') # TO DO: HANDLING OF IF ID IS AT THE END OF FILE NAME
#        subj_base=$(echo "$subject_id_in_subject_file" | cut -d'_' -f1)  # e.g. sub-BRS0034
   ...
   #if [[ "$subject_id_in_subject_file" == sub-*_* ]]; then
   #     pattern="${subject_id_in_subject_file}.zip"
   # elif [[ "$subject_id_in_subject_file" == sub-* ]]; then
   #     pattern="${subject_id_in_subject_file}_*.zip"
   # else
#        echo "Warning: Unrecognized subject ID format '$subject_id_in_subject_file'. Skipping." | tee -a "$log_file"
#        continue
#    fi
...
#                eval $cmd 2>&1 | tee -a "$log_file"
                ...
                if [ $REFACE -eq 1 ]; then
                    ...
                    subject_session_dir="/data/storage/projects/${PROJECT_NAME}/mri/defaced/sub-${subj_id}/ses-${date_yyyymmdd}"
                        ...
                        if [ "${#nii_gz_files[@]}" -eq 0 ]; then
                            ...
                            for nii_gz_file in "${nii_gz_files[@]}"; do
                                ...
                                if [ $should_reface -eq 1 ]; then
                                    ...
                                    echo "Copying non-defaced anat file: $base_name"
                            ...
                            # Move refaced output to anat/ subdirectory in defaced storage
                            ...
                            # Copy other original files (e.g., dwi/, fmap/, etc.) into the defaced session directory
                            ...
                            # Copy any JSON files from anat/ into defaced anat/
                ...
                if [ $MEG_COPY -eq 1 ] || [ $BIDS_MEG -eq 1 ]; then
                    ...
                    digits=$(echo "$subj_no_sub" | sed -E 's/^[a-z]+([0-9]+).*/\1/')
                    meg_subj_dir="${prefix}_${digits}"

                    subj_meg_path="${MEG_DIR}/${meg_subj_dir}"
                    date_folder="${date_yyyymmdd:2}"  # YYMMDD
                    date_folder_path="${subj_meg_path}/${date_folder}"

                    if [ ! -d "$subj_meg_path" ]; then
                        echo "Warning: MEG subject directory '$subj_meg_path' not found for subject '$subj_id'. Skipping MEG." | tee -a "$log_file"
                        continue
                    fi

#                    if [ ! -d "$date_folder_path" ]; then
#                        echo "Warning: Date folder '$date_folder_path' not found for subject '$subj_id'. Skipping MEG." | tee -a "$log_file"
#                        continue
#                    fi
        ...
        echo "No MRI zip files found for pattern '$pattern' in $MRI_DIR" | tee -a "$log_file"
        continue
    fi
...
echo "Cleaning up intermediate MRI folders..."
